...
class PlacesViewModel: ObservableObject {
    ...
    private let apiKey = PlacesViewModel.getApiKey()  // APIキーを取得する関数を使用
    ...
    func fetchPlaces(for location: CLLocation) {
        ...
        // キャッシュキーを生成
        let cacheKey = "\(selectedCategory)_\(searchRadius)_\(location.coordinate.latitude)_\(location.coordinate.longitude)"

        // キャッシュにデータがある場合は、キャッシュからデータを取得
        if let cachedPlaces = cache[cacheKey] {
            self.places = cachedPlaces
            return
        }
        // APIリクエスト数をインクリメント
           apiRequestCount += 1
           print("APIリクエスト数: \(apiRequestCount)")
        ...
        cancellationToken = URLSession.shared.dataTaskPublisher(for: url)
            ...
            .sink(receiveCompletion: { completion in
                ...
                // キャッシュにデータを保存
                self?.cache[cacheKey] = response.places
                                    ...
                                    if let photos = place.photos, let photoReference = photos.first?.photoReference {
                                        VStack {
                                            ZStack(alignment: .topTrailing) {
                                                let apiKey = PlacesViewModel.getApiKey()
                    ...
                    ScrollView(.horizontal, showsIndicators: false) {
                        HStack {
                            ForEach(placeDetailsViewModel.placeDetails?.photos ?? [], id: \.photoReference) { photo in
                                let apiKey = PlacesViewModel.getApiKey()
                                let imageURL = "https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=\(photo.photoReference)&key=\(apiKey)"
        ...
        class PlaceDetailsViewModel: ObservableObject {
            ...
            func fetchPlaceDetails(for placeId: String) {
                  ...
                  urlComponents.queryItems = [
                      ...
                      URLQueryItem(name: "key", value: apiKey),
                      URLQueryItem(name: "fields", value: "name,formatted_phone_number,website,opening_hours,formatted_address")
                  ]
